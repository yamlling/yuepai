  ...
  <div class="home" :class="{homeClose:isCollapse}" @click="homeClick">
    <div class="el-row">
      ...
      <div class="el-col el-col-1 close-col">
        <div style="color:#fff;" @click="loginout()">退出</div>
      ...
      <el-menu :default-active="headerCurRouter" class="el-menu-vertical-demo" background-color="#eef1f6" :collapse="isCollapse" :router="true">
        <template v-for="item in getMenu" v-if="item.meta.show">
          <el-submenu :key="item.path" :index="item.path" v-if="item.meta.showSub">
            ...
            <el-menu-item-group>
              <el-menu-item v-for="child in item.children" :key="child.path" v-if="child.meta.show" :index="item.path + '/' + child.path">
            ...
            },
            {
              validator: (rule, value, callback) => {
                if (!common.pass_reg.test(value)) {
                  ...
                  _this.password.form.pass_word &&
                    _this.$refs.password.validateField("pass_word");
                  callback();
                }
              },
              trigger: "change"
            ...
            },
            {
              validator: (rule, value, callback) => {
                ...
                if (!common.pass_reg.test(value)) {
                  ...
                  form.pass_words &&
                    _this.$refs.password.validateField("pass_words");
                  callback();
                }
              },
              trigger: "change"
            ...
            (data, err) => {
              if (!err) {
                utils.storage.get("userInfo", obj => {
                  ...
                  Array.from(
                    document.querySelectorAll(".top_right p,.user_info dt")
                  ).forEach(d => {
...
.homeClose {
  .el-row {
    .el-col {
      .fa-bars {
        .rotate(90deg);
        animation: fa_close 0.2s;
      }
    }
    .top_left {
      width: 58px;
      animation: left_close 0.2s;
    }
  }
  .content {
    margin-left: 64px;
    width: ~"calc(100% - 64px)";
    animation: content_close 0.3s;
  }
}
